...
## 5. 消融实验与超参数分析 (Ablation Study)
为了系统地验证本方案中各创新模块的贡献，并为超参数选择提供依据，我们进行了详细的消融实验（详见 [`reports/Ablation_Study_Report.md`](Ablation_Study_Report.md)）。

### 5.1 创新模块贡献 (Component Contributions)
| 实验模式 | 平均 PSNR (dB) | 平均 SSIM | 贡献分析 |
| :--- | :--- | :--- | :--- |
| **Baseline (Global DCT)** | 16.40 | 0.4503 | 基础对照组，无重叠，全图压缩，块效应严重。 |
| **+ Overlap** | 19.46 | 0.6304 | 引入重叠分块，SSIM 显著提升 (+0.18)，视觉平滑度改善。 |
| **+ ROI** | 85.36 | 0.9917 | **关键跃升**。引入 ROI 保护后，PSNR 暴涨 (+65dB)，确立了“医疗级无损”的基础。 |
| **+ Dictionary (Proposed)** | **86.84** | **0.9948** | 最终方案。引入 K-SVD 字典，在 ROI 基础上进一步提升背景质量 (+1.5dB)，细节更真实。 |

![Component Comparison](images/ablation_visual_comparison.png)

### 5.2 测量率选择 (Measurement Ratio)
我们在 [0.1, 0.9] 范围内测试了测量率 (MR) 对重构质量的影响。结果显示，当 **MR=0.75** 时，PSNR 增长趋于平缓（拐点），此时背景重构质量已接近无损，且 SSIM 高达 0.9948。考虑到存储效率与质量的平衡，本项目最终选择 **MR=0.75** 作为默认参数。

## 6. 区块链确权与Pipeline Commit
